                    ...
                    return <Row key={index} gutter={{xs:8,sm:16}} justify="center">
                        {
                            createEmptySpace(row).map((element,index2)=>{
                                return (element===undefined?<Col key={index+index2.toString()} style={{height: '50px'}} xs={2} md={1}/>:<Col key={element.id} xs={2} md={1}><SeatItem key={element.id} reserveSeat={reserveSeat}  unreserveSeat={unreserveSeat} data={element}/></Col>)
                            })
                        }
                    </Row>
                })
